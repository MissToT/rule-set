...
if adguard_urls or "adguard" in adblock_cfg:  
...
if "sing-box" in adblock_cfg or "singbox" in adblock_cfg:  
    ...
    # 合并本地 include（后缀域名补 +. 前缀，与 mihomo 分支保持一致的语义）  
...
def main():
            ...
            for i, url in enumerate(url_list):
                try:
                    ...
                    if action_type == "include":
                        ...
                        base_domain_set -= d_set
                        base_ip_set = subtract_ip_networks(base_ip_set, ip_set)
                        base_domain_regex -= dr_set
                except Exception as e:
    ...
    pass_count = 0
    max_passes = 10
    ALL_CATEGORIES = ["domain", "ipcidr", "classical"]
        ...
        for rule_type, rule_name, rule_config in all_rule_defs:
            ...
            for target_str in current["inline_include"]:
                ...
                if has_prefix:
                    ...
                    t_type, names_str = None, target_str
                ...
                for t_name in target_names:
                    ...
                    for target_key in target_keys:
                        if target_key in rule_cache:
                            ...
                            current["domain"] -= target_data["domain"]
                            current["ip"] = subtract_ip_networks(current["ip"], target_data["ip"])
                            current["regex"] -= target_data["regex"]
